...
function check_packages_list() {
  ...
  'curl' 'zip' 'unzip' 'libnet-ldap-perl' 'ldap-utils' \
...
function system_test() {
  ...
  if [[ $(check_php) -eq 0 ]]; then
    ...
    #sudo apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 71DAEAAB4AD4CAB6
    ...
    sudo a2dismod php7.*; sudo a2enmod php7.3; echo "ServerName 127.0.0.1" >> /etc/apache2/apache2.conf; sudo service apache2 restart
    if [[ $(check_php) -eq 0 ]]; then
